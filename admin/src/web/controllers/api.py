...
def get_my_quotas():
    ...
    quotas_list = quotaV2.get_quotas_by_state_by_associated_id(associated_id, False)
    ...
    for q in quotas_list:
        quota_parsed = quotaV2.uotaJson.fromJSON(q.dataJSON)
        quotas_json.append(
            {
                "id": q.id,
                "end_date": q.end_date,
...
def add_payment():
    ...
    quota = quotaV2.get_quota_by_id(int(quota_id))
    ...
    total_amount = quotaV2.QuotaJson.fromJSON(quota.dataJSON).total_amount
    ...
    quotaV2.assign_payment(quota, payment)
...
def get_associated_by_id(nro_associated):
    ...
    if associated:
        ...
        defeated_quotas = quotaV2.get_quotas_by_state_by_associated_id(associated.id, False)
        associated_info = {
...
def associated_activity():
    ...
    for each in quotas_ret:
        data = quotaV2.QuotaJson.fromJSON(each.dataJSON)
        monthly_activity[
