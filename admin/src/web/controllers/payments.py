...
def filtrar_quotas(current_page):
    ...
    associates_list = associates.list_associated()
    quotas_list = []
    quotas = quotaV2.get_quotas()
    for i in quotas:
        if i.associated_id == associated_id and i.state == False:
            quota_parsed = quotaV2.QuotaJson.fromJSON(i.dataJSON)
...
def pagar(current_page, quotaId):
    quota = quotaV2.get_quota_by_id(quotaId)
    quota_detail = quotaV2.get_detail_quota(quotaId)
...
def pagar_post(current_page, quotaId):
    ...
    quotaV2.assign_payment(quota_from_db, payment)
...
def recibo(current_page, payment_id):
    ...
    currency_type = quotaV2.QuotaJson.fromJSON(payment.quotas[0].dataJSON).currency_type
    return render_template(
